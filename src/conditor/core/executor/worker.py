...
class Executor:
    ...
    The executor expects a `step_handler` callable with signature `handler(step: BuildStep) -> Any`.
    The handler is responsible for performing the actual platform/API actions (Discord, etc.).
    """
    ...
    def run_plan(self, plan: BuildPlan, step_handler: Callable[[BuildStep], Any], resume: bool = True):
        ...
        for i in range(start_index, len(plan.steps)):
            ...
            class Executor:
                ...
                The executor expects a `step_handler` callable with signature `handler(step: BuildStep) -> Any`.
                The handler may be async; the executor will await it when needed.
                """
                ...
                async def run_plan(self, plan: BuildPlan, step_handler: Callable[[BuildStep], Awaitable[Any]] | Callable[[BuildStep], Any], resume: bool = True):
                    ...
                    for i in range(start_index, len(plan.steps)):
                        ...
                        retries = int(step.retry_policy.get('retries', 0))
                        backoff = float(step.retry_policy.get('backoff', 2))
                        attempt = 0
                        last_exc = None
